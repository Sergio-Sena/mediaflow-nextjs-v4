...
def lambda_handler(event, context):
    try:
        ...
        if method == 'POST':
            return share_content(event, user)
        elif method == 'DELETE':
...
def unshare_content(event, user):
    ...
    return cors_response(200, {'success': True, 'message': 'Content removed from public'})


def get_public_content(event, user):
    """List all public content."""
    # Scan all active content
    resp = table.scan(
