...
def lambda_handler(event, context):
    ...
    _request_origin = get_allowed_origin(event) if event.get('headers') else 'https://midiaflow.sstechnologies-cloud.com'
    try:
...
def manual_cleanup():
    ...
    try:
        ...
        return {
            ...
            'body': json.dumps({'success': True, 'cleaned': cleaned, 'count': len(cleaned)})
        }
    except Exception as e:
        return {
            'statusCode': 500,
