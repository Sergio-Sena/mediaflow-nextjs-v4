#!/usr/bin/env python3
"""
Comparar arquivos locais com usuario sergio_sena no S3
"""
...
def get_local_files(local_path):
    ...
    for dirpath, dirnames, filenames in os.walk(local_path):
        for filename in filenames:
            if filename.lower().endswith(('.mp4', '.avi', '.mkv', '.mov', '.ts')):
...
def main():
    ...
    if missing_in_s3:
        ...
        for folder, files in missing_by_folder.items():
            ...
            for file in files:
                print(f"  {file['filename']} ({file['size']:.1f} MB)")
        ...
        # Sugestao de upload
        print(f"\nSUGESTAO:")
        print(f"Fazer upload destes {len(missing_in_s3)} arquivos para users/sergio_sena/")
        
    else:
        print("TODOS OS ARQUIVOS LOCAIS JA EXISTEM EM SERGIO_SENA!")
        print("Nenhum upload necessario.")
    
    # Estatisticas adicionais
    print(f"\nESTATISTICAS:")
    print(f"Cobertura: {((len(local_files) - len(missing_in_s3)) / len(local_files) * 100):.1f}%" if local_files else "N/A")
